angular.module('vms.routing').controller('VMSIndexController', function($scope, $http, pageTitle, gettext, notify) {
    ...
    $http.get('/api/routing').then( (resp) => {
	    $scope.python_get = resp.data;
    });

    // POST a result through Python API
    $http.post('/api/routing', {my_var: 'my_plugin'}).then( (resp) => {
